...
def save_model_scores_v1(model_name, output_dir, interface_weight):
    """
    ...
    rp = open(os.path.join(model_output_dir, model_name + '.model_scores'), 'w')
    ...
    rp.close()
def save_model_scores_v2(model_name, output_dir, interface_weight):
    """
    ...
    for model in models:
        ...
        if checkR:
            ...
            rp.write(model + '\t' + str(ips) + '\t' + str(ics) + '\t' + str(qsbest) + '\t' + str(dockq) + '\t' + str(lddt) + '\t' + str(tm) + '\n')
    rp.close()
def save_model_scores_v3(model_name, output_dir, interface_weight):
    """
